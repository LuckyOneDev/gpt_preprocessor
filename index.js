...
(async () => {
    try {
        ...
        await Promise.all(inputFiles.map(inputFile => processFile(path.resolve(inputFile), outputFile, tsConfig, projectFolder)));
 ...
 */
async function readTsConfig(projectFolder) {
    if (tsConfigCache)
 ...
 */
async function processFile(filePath, outputFile, tsConfig, projectFolder) {
 ...
 */
async function compressOutput(outputFile) {
 ...
 */
async function resolveImportPath(currentFile, importPath, tsConfig, projectFolder) {
...
async function resolveModulePathUsingTsconfig(moduleName, baseUrl, paths) {
 ...
 */
async function findFileWithExtensions(filePath) {
